...
const Calificaciones = () => {
  ...
  const cargarEstudiantes = async () => {
    try {
      setLoading(true);
      // Buscar el grupo en la lista ya cargada (que incluye estudiantes)
      const grupoSeleccionado = grupos.find(g => g.id === parseInt(selectedGrupo));
      const estudiantesDelGrupo = grupoSeleccionado?.estudiantes || [];
      setEstudiantes(estudiantesDelGrupo);
      setEstudianteIndex(0);
      
      if (estudiantesDelGrupo.length > 0) {
        setSelectedEstudiante(estudiantesDelGrupo[0].id);
      } else {
        setSelectedEstudiante('');
        toast.info('Este grupo no tiene estudiantes asignados');
      }
    } catch (error) {
  ...
  const cargarLogrosYCalificaciones = async () => {
    try {
      ...
      const logrosConCalificaciones = logros.map(logro => {
        const calif = calificaciones.find(c => c.logro?.id === logro.id);
        return {
          logroId: logro.id,
          nombre: logro.nombre,
          categoria: logro.categoria || 'Sin categoría',
          calificacionId: calif?.id || null,
          nota: calif?.valor || null
        };
      });
  ...
  const handleSaveCalificacion = async (logro, nota) => {
    try {
      ...
      if (logro.calificacionId) {
        ...
        await api.put(`/calificaciones/${logro.calificacionId}`, {
          valor: nota,
          periodo,
    ...
    try {
      ...
      const promises = logrosConCalif
        ...
        .map(async (logro) => {
          ...
          if (logro.calificacionId) {
            return api.put(`/calificaciones/${logro.calificacionId}`, {
              valor: nota,
              periodo,
              logroId: logro.logroId,
              estudianteId: parseInt(selectedEstudiante)
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      const response = await api.get(
        `/calificaciones/reporte/boletin?estudianteId=${selectedEstudiante}&periodo=${periodo}`,
        { responseType: 'blob' }
      );
...
export const ProfesorCalificaciones = Calificaciones;
